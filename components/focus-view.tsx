    ...
    <div className="flex flex-col items-center animate-fade-in" key={`${task.id}-${displayIndex}`}>
      ...
      <div className="mb-5">
        <ProgressRing progress={progressPct} size={64}>
          ...
          <span className="text-[10px] text-muted">/{stats.total}</span>
        </ProgressRing>
      </div>
        ...
        {hasProtocol && (
          <div className="border-t border-border-subtle">
            ...
            >
              ...
              <svg
                className={`w-3 h-3 transition-transform duration-200 ${showProtocol ? 'rotate-180' : ''}`}
                fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}
              >
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            ...
            {showProtocol && (
              <div className="px-5 pb-4 animate-slide-down">
                <div className="space-y-2">
                  {task.protocol!.actions.map((action, i) => (
                    <div key={i} className="flex items-start gap-2.5">
                      <span className="shrink-0 w-5 h-5 rounded-full bg-emerald-500 text-white text-[10px] font-bold flex items-center justify-center mt-0.5">
                        {i + 1}
                      </span>
                      <p className="text-[12px] text-foreground leading-relaxed">
                        {action}
      ...
      <div className="flex items-center gap-3 mt-5 w-full">
        <button
          ...
          className="flex-1 py-3.5 rounded-2xl text-[14px] font-semibold text-muted bg-surface border border-border hover:bg-surface-hover active:scale-[0.98] transition-all duration-150 tap-highlight flex items-center justify-center gap-2"
        >
        ...
        <button
          onClick={handleDone}
          className="flex-1 py-3.5 rounded-2xl text-[14px] font-semibold text-white bg-accent hover:brightness-110 active:scale-[0.98] shadow-sm shadow-accent/20 transition-all duration-150 tap-highlight flex items-center justify-center gap-2"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
